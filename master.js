...
try {
  if (!fs.existsSync('./test.log')) {
    ...
    fs.writeFile(path.join(__dirname, fileName), 'This is Log', err => {
      if (err) throw err;
...
socket.on('image', data => {
  try {
    ...
    fs.appendFile(
      ...
      `\n${data.index}. Image: ${date}.png created`,
      err => {
        if (err) console.log(err);
...
socket.on('json', data => {
  try {
    ...
    const callback = function(body) {
      fs.appendFile(
        path.join(__dirname, fileName),
        `\n${data.index}. ${data.data.query} meets ${body.search_information.total_results} times`,
        err => {
          if (err) console.log(err);
          console.log(
